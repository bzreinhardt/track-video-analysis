...
motorData.time = motorData.time - motorData.time(spinStartInd); %synch the time of the motor data

stopInd = find(motorData.time > t_stop,1); 
startInd = find(motorData.time > t_start,1);

fields = fieldnames(motorData);

% extract only the data for the interval of interest from the motor data
